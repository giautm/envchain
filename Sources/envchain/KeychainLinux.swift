...
struct Keychain: SecretBackend {
  ...
  nonisolated(unsafe) private static let schema:
    UnsafeMutablePointer<SecretSchema> = {
      ...
      guard let name = strdup("org.envchain.item"),
        ...
        let attrAccount = strdup("account")
      else {
  ...
  func searchValues(
    ...
    while let item = current?.pointee.data {
      ...
      let accountStr = String(
        cString: account.assumingMemoryBound(to: CChar.self))
  ...
  func searchNamespaces() -> [String] {
    ...
    while let item = current?.pointee.data {
      ...
      guard let service = g_hash_table_lookup(attrs, "service") else {
        continue
      }
      let serviceStr = String(
        cString: service.assumingMemoryBound(to: CChar.self))
      guard serviceStr.hasPrefix(servicePrefix) else {
        continue
      }
